...
3. **Set the autonomy level:** `gh variable set ESTATE_AUTONOMY -b assist -R frankxai/<repo>`
   - `off` — classify + status only, zero cost.
   - `assist` — review, label, fix on `@estate fix`. **Recommended default.**
   - `auto` — additionally auto-merges the safe tier (TRIVIAL/STANDARD, green, approved). Only for low-blast-radius repos.
