...
>
**Last sweep:** 2026-07-16 · **Cadence:** end of each working session (`/ops-sweep`)

**2026-07-28 SMIS knowledge governance:** `ops/media-intelligence/` is the private media decision/control plane. Its reviewed strategy, current execution program, source-scoped research ledger, ownership graph, and capture protocol are the entry points for media-intelligence agents; see `ops/media-intelligence/README.md`. SIS remains canonical for durable provenance/lessons, ACOS for only proven portable workflows, brand repos for editorial bodies/assets, and vendors as replaceable inputs. No publishing/OAuth/scheduler capability is enabled by this documentation update.
